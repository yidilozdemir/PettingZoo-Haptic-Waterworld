...
class WaterworldBase:
    ...
    # Add this new method to the WaterworldBase class
    def calculate_proximity_rewards(self):
        proximity_rewards = np.zeros(self.n_pursuers)
        for i in range(self.n_pursuers):
            for j in range(i + 1, self.n_pursuers):
                distance = np.linalg.norm(
                    np.array(self.pursuers[i].body.position) - 
                    np.array(self.pursuers[j].body.position)
                ) / self.pixel_scale  # Normalize by pixel_scale
                if distance < self.proximity_threshold:
                    proximity_rewards[i] += self.proximity_reward
                    proximity_rewards[j] += self.proximity_reward
        return proximity_rewards
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            ...
            proximity_rewards = self.calculate_proximity_rewards()
            ...
            for id in range(self.n_pursuers):
                ...
                self.behavior_rewards[id] = (
                    ...
                    + proximity_rewards[id]  # Add proximity reward
                )
